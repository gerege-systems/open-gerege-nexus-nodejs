...
export function describeError(err, fallback) {
    if (err instanceof EsignApiError)
        return err.message;
...
export function Banner({ tone, message, onDismiss, }) {
    ...
    return (<div className={`p-3 border text-sm rounded-lg flex items-start gap-2 ${style}`}>
      {tone === "error" ? (<AlertTriangle className="w-4 h-4 mt-0.5 shrink-0"/>) : (<CheckCircle2 className="w-4 h-4 mt-0.5 shrink-0"/>)}
      <span className="flex-1">{message}</span>
      {onDismiss && (<button onClick={onDismiss} aria-label={t("base.action.close")}>
          <X className="w-4 h-4"/>
        </button>)}
    </div>);
}
...
export function Card({ title, children, actions }) {
    return (<section className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
      {(title || actions) && (<header className="px-4 py-3 border-b border-slate-200 flex items-center justify-between gap-3">
...
export function Badge({ tone, children }) {
    return (<span className={`inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-0.5 rounded-full border ${tone}`}>
      {children}
    </span>);
}
/**
...
export function SessionBadge({ state }) {
 ...
 */
export function Pager({ total, offset, pageSize, onPage, }) {
    const { t } = useI18n();
    if (total <= pageSize)
        return null;
    const page = Math.floor(offset / pageSize) + 1;
    const pages = Math.ceil(total / pageSize);
    return (<nav className="flex items-center justify-between text-xs text-slate-500">
      <span>{t("base.message.page_summary", { page, pages, total })}</span>
      <div className="flex gap-2">
        <button onClick={() => onPage(Math.max(0, offset - pageSize))} disabled={offset === 0} className="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 disabled:opacity-40 font-medium">
          {t("base.action.previous")}
        </button>
        <button onClick={() => onPage(offset + pageSize)} disabled={offset + pageSize >= total} className="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 disabled:opacity-40 font-medium">
          {t("base.action.next")}
        </button>
      </div>
    </nav>);

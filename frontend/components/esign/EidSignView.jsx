...
import { Building2, Clock, Download, FileText, PenLine, RotateCcw, ShieldCheck, Smartphone, Upload, UserRound, } from "lucide-react";
...
export default function EidSignView({ onSigned }) {
    ...
    const orgLabel = useCallback((org) => (locale === "en" && org.org_name_en ? org.org_name_en : org.org_name), [locale]);
    useEffect(() => {
        let active = true;
        esign
            .organizations()
    ...
    const watch = useCallback(async (session, orgName) => {
    ...
    async function start(file, orgName, signer) {
    ...
    async function downloadSigned(session) {
        try {
    ...
    return (<>
      <input ref={fileRef} type="file" accept="application/pdf,.pdf" className="hidden" onChange={onFile} aria-label={t("esign.action.pick_pdf")}/>

      {phase.kind === "idle" && (<section className="bg-white border border-slate-200 rounded-xl shadow-sm">
          ...
          <div className="px-4 py-8 text-center">
            <div className="max-w-sm mx-auto mb-6 text-left">
              <label htmlFor="esign-onbehalf" className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-slate-500">
                <Building2 className="w-3.5 h-3.5"/>
                {t("esign.field.sign_as")}
              </label>
              <select id="esign-onbehalf" value={orgEtsi} onChange={(event) => setOrgEtsi(event.target.value)} disabled={orgsLoading} className="mt-1.5 w-full px-3 py-2 text-sm border border-slate-300 rounded-lg bg-white focus:ring-2 focus:ring-indigo-500 disabled:bg-slate-50">
                <option value="">{t("esign.field.sign_as_self")}</option>
                {orgs.map((org) => (<option key={org.org_etsi} value={org.org_etsi}>
                    {orgLabel(org)}
            ...
            <button type="button" onClick={() => fileRef.current?.click()} className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold px-5 py-2.5 rounded-lg inline-flex items-center gap-2 shadow-sm transition">
      ...
      {phase.kind === "identity" && (<section className="bg-white border border-slate-200 rounded-xl shadow-sm">
          <header className="px-4 py-3 border-b border-slate-200 flex items-center gap-2">
            <UserRound className="w-4 h-4 text-indigo-600"/>
            <h2 className="text-sm font-bold text-slate-800">{t("esign.view.signer_identity")}</h2>
          </header>

          <form className="px-4 py-6" onSubmit={(event) => {
                event.preventDefault();
                if (signerId.trim())
                    void start(phase.file, phase.orgName, signerId);
            }}>
            ...
            <div className="max-w-sm mx-auto mt-5">
              ...
              <input id="esign-signer-id" value={signerId} onChange={(event) => setSignerId(event.target.value.toUpperCase())} placeholder={t("esign.field.signer_id_placeholder")} autoFocus className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500"/>
            ...
            <div className="mt-5 flex gap-2.5 justify-center flex-wrap">
              <button type="submit" disabled={!signerId.trim()} className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white text-sm font-semibold px-5 py-2.5 rounded-lg">
                {t("esign.action.continue_signing")}
              </button>
              <button type="button" onClick={reset} className="bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-medium px-4 py-2 rounded-lg inline-flex items-center gap-1.5">
      ...
      {phase.kind === "uploading" && (<section className="bg-white border border-slate-200 rounded-xl shadow-sm p-8 text-center">
      ...
      {phase.kind === "waiting" && (<section className="bg-white border border-slate-200 rounded-xl shadow-sm p-8 text-center">
          ...
          {phase.orgName && (<p className="text-sm text-indigo-600 mt-1 flex items-center justify-center gap-1.5">
              <Building2 className="w-3.5 h-3.5"/>
          ...
          {phase.session.verification_code && (<>
              ...
              <div className="flex justify-center gap-2.5 mt-2">
                {phase.session.verification_code.split("").map((digit, index) => (<span key={index} className="w-11 h-14 inline-flex items-center justify-center text-2xl font-bold font-mono text-indigo-600 bg-slate-100 rounded-xl">
                    {digit}
                  </span>))}
              </div>
            </>)}
          ...
          <button type="button" onClick={() => cancel(phase.session)} className="mt-5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-medium px-4 py-2 rounded-lg inline-flex items-center gap-1.5">
            <RotateCcw className="w-3.5 h-3.5"/>
            {t("base.action.cancel")}
          </button>
        </section>)}

      {phase.kind === "completed" && (<section className="bg-white border border-slate-200 rounded-xl shadow-sm p-8 text-center">
          ...
          {phase.session.certificate_level && (<p className="text-xs text-slate-500 mt-2 font-mono">{phase.session.certificate_level}</p>)}

          <div className="mt-5 flex gap-2.5 justify-center flex-wrap">
            <button type="button" onClick={() => downloadSigned(phase.session)} className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold px-4 py-2 rounded-lg inline-flex items-center gap-2">
      ...
      {phase.kind === "error" && (<section className="bg-white border border-slate-200 rounded-xl shadow-sm p-8 text-center">
          <p className="font-semibold text-red-600">{phase.message}</p>
          <button type="button" onClick={reset} className="mt-4 bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-medium px-4 py-2 rounded-lg inline-flex items-center gap-1.5">
            <RotateCcw className="w-3.5 h-3.5"/>
            {t("base.action.retry")}
          </button>
        </section>)}

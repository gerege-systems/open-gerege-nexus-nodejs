...
function deadlineOf(start) { const at = Date.parse(start.expires_at ?? ""); return Number.isNaN(at) ? Date.now() + BACKSTOP : at; }
function hasDeadline(start) { return !Number.isNaN(Date.parse(start.expires_at ?? "")); }
function sleep(ms) { return new Promise(resolve => setTimeout(resolve, ms)); }
function clock(seconds) { return `${Math.floor(seconds / 60)}:${String(seconds % 60).padStart(2, "0")}`; }
export default function EIDLogin({ next = "/apps", compact = false }) {
    const { t } = useI18n();
    const [method, setMethod] = useState("id"), [phase, setPhase] = useState("idle"), [nationalId, setNationalId] = useState(""), [start, setStart] = useState(null), [error, setError] = useState(""), [left, setLeft] = useState(0);
    ...
    const ticket = useRef(0), inflight = useRef(null);
    ...
    const watch = useCallback(async (data) => {
        stop();
        ...
        while (ticket.current === mine) {
            ...
            try {
                ...
                if (++failures > TOLERATED_FAILURES) {
                    setError(e?.message || t("auth.message.error_link"));
                    setPhase("error");
                    return;
                }
            }
    ...
    const begin = useCallback(async (selected) => {
        stop();
        ...
        try {
            ...
            if (ticket.current !== mine)
                return;
            if (selected === "qr" && mobile())
                window.location.href = `geregesmartid://approve?sessionId=${encodeURIComponent(data.session_id)}`;
            void watch(data);
        }
        catch (e) {
            if (ticket.current !== mine)
                return;
            setError(e?.message || t("auth.message.error_service"));
            setPhase("error");
        }
    ...
    useEffect(() => {
        ...
        const tick = () => { const secs = Math.max(0, Math.ceil((deadline - Date.now()) / 1000)); setLeft(secs); if (secs === 0) {
            stop();
            setPhase("expired");
        } };
        tick();
    ...
    const switchMethod = (value) => { stop(); setMethod(value); setPhase("idle"); setStart(null); setError(""); if (value === "qr")
        void begin("qr"); };

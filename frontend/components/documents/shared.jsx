...
import { AlertTriangle, Ban, CheckCircle, Clock, FileText, History, Loader2, PenLine, Send, ShieldCheck, Users, X, XCircle, } from "lucide-react";
...
function statedDeadline(expiresAt) {
    const at = Date.parse(expiresAt ?? "");
    return Number.isNaN(at) ? null : at;
}
function deadlineOf(expiresAt) {
    const stated = statedDeadline(expiresAt);
    if (stated === null)
        return Date.now() + APPROVAL_BACKSTOP;
...
export function StatusBadge({ status }) {
    ...
    if (status === "APPROVED") {
        return (<span className={`${shell} bg-emerald-50 text-emerald-700 border border-emerald-200`}>
        <CheckCircle className="w-3 h-3 text-emerald-500"/>
        <span>{t("documents.state.approved")}</span>
      </span>);
    }
    if (status === "REJECTED") {
        return (<span className={`${shell} bg-red-50 text-red-700 border border-red-200`}>
        <XCircle className="w-3 h-3 text-red-500"/>
        <span>{t("documents.state.rejected")}</span>
      </span>);
    }
    if (status === PENDING) {
        return (<span className={`${shell} bg-amber-50 text-amber-700 border border-amber-200`}>
        <Clock className="w-3 h-3 text-amber-500"/>
        <span>{t("documents.state.pending")}</span>
      </span>);
    }
    if (status === "DRAFT") {
        return (<span className={`${shell} bg-slate-100 text-slate-600 border border-slate-200`}>
 ...
 */
export function SignatureProgress({ doc }) {
    const { t } = useI18n();
    if (doc.required_signatures <= 1)
        return null;
    ...
    const complete = doc.status !== "REJECTED" &&
        doc.outstanding_steps === 0 &&
        doc.signature_count >= doc.required_signatures;
    return (<span className={`inline-flex items-center space-x-1 text-[11px] font-semibold px-2 py-0.5 rounded-full border ${complete
            ? "bg-emerald-50 text-emerald-700 border-emerald-200"
            : "bg-indigo-50 text-indigo-700 border-indigo-200"}`} title={t("documents.message.signature_progress", {
...
export function SignatureCell({ doc }) {
    const { t } = useI18n();
    if (doc.signed_by) {
        ...
        {doc.signature_hash && (<div className="font-mono text-[10px] text-slate-400 truncate max-w-[220px]" title={doc.signature_hash}>
...
export function Banner({ message, onDismiss }) {
    const { t } = useI18n();
    const error = message.type === "error";
    return (<div className={`p-4 rounded-lg flex items-start space-x-3 text-sm font-medium border ${error ? "bg-red-50 border-red-200 text-red-800" : "bg-emerald-50 border-emerald-200 text-emerald-800"}`}>
      {error ? (<AlertTriangle className="w-5 h-5 text-red-600 shrink-0"/>) : (<CheckCircle className="w-5 h-5 text-emerald-600 shrink-0"/>)}
...
export function useDocumentActions(onChanged) {
    ...
    const setBusyId = (id, working) => setBusyIds((current) => {
    ...
    const reject = async (doc) => {
        if (!confirm(t("documents.message.reject_confirm", { title: doc.title })))
            return;
        setBusyId(doc.id, true);
        setMessage(null);
        try {
...
export function RowActions({ doc, busy, canSign, canManage, onSign, onReject, onRoute, }) {
    const { t } = useI18n();
    ...
    if (doc.status === "DRAFT") {
        if (!canManage || !onRoute)
            return <span className="text-slate-300 text-[11px]">—</span>;
        return (<button onClick={() => onRoute(doc)} disabled={busy} className="inline-flex items-center space-x-1 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold border border-slate-300 text-slate-700 hover:bg-slate-50 transition disabled:opacity-50">
    ...
    return (<div className="flex items-center justify-end space-x-2">
      <button onClick={() => onSign(doc)} disabled={busy} className="inline-flex items-center space-x-1 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold border border-indigo-200 text-indigo-600 hover:bg-indigo-50 transition disabled:opacity-50">
        <PenLine className="w-3.5 h-3.5"/>
        <span>{t("documents.action.sign")}</span>
      </button>
      <button onClick={() => onReject(doc)} disabled={busy} className="inline-flex items-center space-x-1 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold border border-red-200 text-red-600 hover:bg-red-50 transition disabled:opacity-50">
 ...
 * showing them.
 */
export function SignatureDialog({ doc, onClose, onDone, onError, }) {
    const { t } = useI18n();
    const [method, setMethod] = useState("EID");
    ...
    return (<div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-xl border border-slate-200">
        ...
        {failure && (<div className="mb-4 p-3 rounded-lg border border-red-200 bg-red-50 text-red-800 text-xs flex items-start gap-2">
        ...
        {session ? (<div className="space-y-4">
            ...
            <div className="flex items-center gap-2 text-sm text-slate-600">
              ...
              {statedDeadline(session.expires_at) !== null && (<span className="font-mono text-xs text-slate-500 tabular-nums">
            ...
            <button type="button" onClick={onClose} className="w-full bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium py-2 rounded-lg text-xs">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {["EID", "DAN"].map((m) => (<button key={m} type="button" onClick={() => setMethod(m)} className={`py-2 rounded-lg text-xs font-semibold border transition ${method === m
            ...
            <div>
              ...
              <input type="text" placeholder="УБ99010111" value={regNumber} onChange={(e) => setRegNumber(e.target.value)} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 font-mono" minLength={8} maxLength={64} required/>
            </div>
            ...
            {method === "DAN" && (<div>
                ...
                <input type="text" placeholder="123456" value={otpCode} onChange={(e) => setOtpCode(e.target.value)} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 font-mono"/>
              </div>)}

            <div className="flex items-center space-x-2 pt-2">
              <button type="button" onClick={onClose} className="w-1/2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium py-2 rounded-lg text-xs">
                {t("base.action.cancel")}
              </button>
              <button type="submit" disabled={busy} className="w-1/2 bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 rounded-lg text-xs disabled:opacity-50">
                {busy
 ...
 */
export function SignatureHistoryDialog({ doc, onClose }) {
    const { t } = useI18n();
    ...
    useEffect(() => {
        ...
        Promise.all([api.getDocumentSignatures(doc.id), api.getDocumentSteps(doc.id)])
            ...
            .catch((err) => alive && setError(err?.message || t("documents.message.history_failed")));
        return () => {
            alive = false;
        };
    ...
    const extraRows = (signatures || [])
        ...
        .map((signature) => ({ step: undefined, signature }));
    ...
    return (<div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl max-w-lg w-full p-6 shadow-xl border border-slate-200">
        ...
        {error ? (<p className="text-sm text-red-700 bg-red-50 border border-red-200 rounded-lg p-3">{error}</p>) : signatures === null ? (<div className="flex items-center gap-2 text-slate-500 text-sm py-6">
            <Loader2 className="w-4 h-4 animate-spin"/>
            {t("documents.message.loading")}
          </div>) : rows.length === 0 ? (<p className="text-sm text-slate-500 py-6">{t("documents.message.no_signatures")}</p>) : (<ol className="space-y-3 max-h-[60vh] overflow-y-auto">
            {rows.map(({ step, signature }, index) => {
                ...
                return (<li key={step ? `step-${step.order}` : `sig-${index}`} className={`border rounded-lg p-3 ${filled
                        ? "border-slate-200"
                        : isNext
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <span className={`w-5 h-5 shrink-0 rounded-full text-[10px] font-bold grid place-items-center ${filled ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>
                          {step ? step.order : index + 1}
                        </span>
                        <p className="text-sm font-semibold text-slate-900 truncate">
                          {signature ? signature.signer_name : step?.name || "—"}
                        </p>
                        {!step && signature && steps.length > 0 && (<span className="shrink-0 text-[10px] text-slate-500 italic">
                    ...
                    {filled ? (<span className="shrink-0 text-[11px] font-bold px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 border border-blue-200">
                        {signature.signer_method === "EID" ? "E-ID" : signature.signer_method}
                      </span>) : (<span className={`shrink-0 text-[11px] font-semibold px-2 py-0.5 rounded-full border ${isNext
                  ...
                  {signature && (<dl className="mt-2 space-y-1 text-[11px] pl-7">
                      ...
                      {signature.certificate_serial ? (<>
                          ...
                          {signature.certificate_issuer && (<div className="flex gap-2">
        ...
        <button type="button" onClick={onClose} className="mt-5 w-full bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium py-2 rounded-lg text-xs">
          {t("base.action.close")}
        </button>
      </div>
    </div>);
...
export function SignatureHistoryButton({ doc, onOpen }) {
    const { t } = useI18n();
    ...
    if (doc.signature_count === 0 && doc.outstanding_steps === 0)
        return null;
    return (<button onClick={() => onOpen(doc)} title={t("documents.action.view_history")} aria-label={t("documents.action.view_history")} className="inline-flex items-center space-x-1 px-2 py-1 rounded-lg text-[11px] font-semibold border border-slate-300 text-slate-600 hover:bg-slate-50 transition">

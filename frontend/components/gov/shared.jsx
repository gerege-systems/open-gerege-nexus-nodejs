...
import { GovApiError } from "@/lib/gov";
...
export const ALL_STATUSES = Object.keys(STATUS_STYLE);
...
export function StatusBadge({ status }) {
    ...
    return (<span className={`text-xs font-semibold px-2 py-0.5 rounded ${STATUS_STYLE[status] || "bg-slate-100"}`}>
      {label(status)}
    </span>);
}
...
export function Banner({ tone, message, onDismiss, }) {
    ...
    return (<div className={`p-3 border text-sm rounded-lg flex items-start gap-2 ${error ? "bg-red-50 border-red-200 text-red-700" : "bg-emerald-50 border-emerald-200 text-emerald-700"}`}>
...
export function DashboardCards({ dashboard }) {
    const { t } = useI18n();
    if (!dashboard)
        return null;
    const cards = [
    ...
    return (<section className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-3">
      {cards.map((card) => (<div key={card.key} className="p-4 bg-white border border-slate-200 rounded-xl">

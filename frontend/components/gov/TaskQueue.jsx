...
import { ACTION_PERMISSION, ACTION_REQUIRES_COMMENT, availableActions, gov, } from "@/lib/gov";
import { useAccess } from "@/lib/access";
import { useI18n } from "@/lib/i18n";
import RequestDrawer from "@/components/gov/RequestDrawer";
import { ALL_STATUSES, Banner, DashboardCards, EmptyState, Loading, StatusBadge, describeError, useStatusLabel, } from "@/components/gov/shared";
 ...
 */
export default function TaskQueue({ showDashboard = true }) {
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <select value={statusFilter} onChange={(e) => {
          ...
          {ALL_STATUSES.map((status) => (<option key={status} value={status}>
              {statusLabel(status)}
            </option>))}
        </select>

        <select value={unitFilter} onChange={(e) => {
            setPage(1);
        ...
        <label className="flex items-center gap-2 text-sm text-slate-600">
          <input type="checkbox" checked={overdueOnly} onChange={(e) => {
            setPage(1);
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        {!queue || queue.items.length === 0 ? (<EmptyState message={t("gov.message.queue_empty")}/>) : (<div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[880px]">
              ...
              <tbody className="divide-y divide-slate-100">
                {queue.items.map((task) => (<tr key={task.id} className={task.overdue ? "bg-red-50/40" : undefined}>
                    <td className="px-4 py-3">
                      <button onClick={() => openDetail(task.application_id)} className="font-mono text-xs text-[var(--gerege-blue)] hover:underline">
                        {task.reference}
                      </button>
                    </td>
                    ...
                    <td className="px-4 py-3 text-xs">
                      {task.due_at ? (<span className={task.overdue ? "text-red-600 font-semibold" : "text-slate-500"}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1.5 justify-end">
                        {busy === task.id ? (<Loader2 className="w-4 h-4 animate-spin text-slate-400"/>) : (availableActions(task.status)
                    .filter((action) => access.can(ACTION_PERMISSION[action]))
                    .map((action) => (<button key={action} onClick={() => act(task, action)} className="text-xs px-2 py-1 rounded border border-slate-200 hover:bg-slate-50 text-slate-600">
                                {t(`gov.action.${action}`)}
                              </button>)))}
                      </div>
                    </td>
      ...
      {queue && queue.total > PAGE_SIZE && (<div className="flex items-center justify-between text-sm text-slate-500">
          <span>
          ...
          <div className="flex gap-2">
            <button disabled={queue.page <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))} className="px-3 py-1.5 border border-slate-200 rounded-lg disabled:opacity-40">
              {t("base.action.previous")}
            </button>
            <button disabled={queue.page * queue.page_size >= queue.total} onClick={() => setPage((p) => p + 1)} className="px-3 py-1.5 border border-slate-200 rounded-lg disabled:opacity-40">
              {t("base.action.next")}
            </button>
          </div>
        </div>)}

      {detail && (<RequestDrawer detail={detail} onClose={() => setDetail(null)} unitName={unitName} statusBadge={(status) => <StatusBadge status={status}/>}/>)}
    </div>);
}

...
export default function RequestDrawer({ detail, onClose, unitName, statusBadge, }) {
    const { t, locale } = useI18n();
    const format = (value) => new Date(value).toLocaleString(locale === "en" ? "en-GB" : "mn-MN");
    return (<div className="fixed inset-0 bg-slate-950/40 flex justify-end z-50" onClick={onClose}>
      <aside onClick={(e) => e.stopPropagation()} className="w-full max-w-xl bg-white h-full overflow-y-auto p-6 space-y-6 shadow-xl">
        ...
        <section className="space-y-2">
          ...
          {detail.tasks.map((task) => (<div key={task.id} className="p-3 border border-slate-200 rounded-lg text-sm flex items-center gap-3">
        ...
        <section className="space-y-3">
          ...
          <ol className="space-y-3">
            {detail.timeline.map((event) => (<li key={event.id} className="flex gap-3 text-sm">
                <span className="mt-1.5 w-2 h-2 rounded-full bg-[var(--gerege-blue)] shrink-0"/>
                <div>
                  <p className="text-slate-800">
                    {event.message || event.event_type}
                    {event.from_status && event.to_status ? (<span className="text-xs text-slate-400">
                        {" "}

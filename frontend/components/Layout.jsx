...
export default function Layout({ children }) {
    const [menus, setMenus] = useState([]), [user, setUser] = useState(null), [loading, setLoading] = useState(true);
    ...
    function togglePanel() { if (window.matchMedia("(min-width:901px)").matches) {
        setPanelOpen(v => { localStorage.setItem("gerege_sidebar_open", String(!v)); return !v; });
    }
    else
        setMobileOpen(v => !v); }
    ...
    catch { } resetAccess(); router.push("/login"); }
    ...
    if (isPublic)
        return <>{children}</>;
    if (loading)
        return <div className="min-h-screen flex items-center justify-center bg-slate-50 text-slate-500 font-medium">{t("web.message.loading_platform")}</div>;
    ...
    <header className="gerege-topbar h-16 flex items-center border-b sticky top-0 z-50">
      ...
      <div className="gerege-header-search hidden md:flex flex-1 items-center justify-center min-w-0 px-5 relative">
        <div className="relative w-full max-w-md"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400"/><input value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && results[0]) {
        router.push(results[0].path);
        setQuery("");
    } }} placeholder={t("web.view.search_placeholder")} className="w-full h-10 rounded-full border border-slate-200 bg-slate-100/80 pl-10 pr-4 text-sm outline-none focus:border-[var(--gerege-blue)] focus:ring-2 focus:ring-[color-mix(in_srgb,var(--gerege-blue)_15%,transparent)]"/>
...
function AppRailLink({ href, active, title, icon }) { return <Link href={href} title={title} aria-label={title} className={`w-11 h-11 rounded-xl grid place-items-center transition ${active ? "bg-[var(--gerege-blue-soft)] text-[var(--gerege-blue)] shadow-sm" : "text-slate-500 hover:bg-[var(--gerege-surface-2)] hover:text-slate-800"}`}>{icon}</Link>; }
function MobileAppTab({ href, active, label, icon }) { return <Link href={href} aria-label={label} aria-current={active ? "page" : undefined} className={`gerege-mobile-tab ${active ? "is-active" : ""}`}><span>{icon}</span><small>{label}</small></Link>; }
function MobileMoreApp({ href, active, label, icon }) { return <Link href={href} aria-current={active ? "page" : undefined} className={`gerege-mobile-more-app ${active ? "is-active" : ""}`}><span>{icon}</span><strong>{label}</strong></Link>; }
function NavLink({ href, active, icon, label }) { return <Link href={href} className={`gerege-nav-link flex items-center gap-3 px-3 py-2.5 text-sm font-medium transition ${active ? "gerege-nav-link-active font-semibold" : ""}`}><span className="gerege-nav-icon">{icon}</span><span>{label}</span></Link>; }
function MenuGroup({ title, children }) { return <section className="mb-6"><h3 className="px-3 mb-2 text-[11px] font-bold uppercase tracking-wider text-slate-400">{title}</h3><div className="space-y-1">{children}</div></section>; }
function AppMenuGroups({ menus, pathname }) { const roots = menus.filter(item => !item.parent_id).sort((a, b) => a.order - b.order); return <>{roots.map(root => <MenuGroup key={root.id} title={root.label}>{menus.filter(item => item.parent_id === root.id && item.path).sort((a, b) => a.order - b.order).map(item => <NavLink key={item.id} href={item.path} active={pathname.startsWith(item.path)} icon={iconMap[item.icon] || <Package className="w-5 h-5"/>} label={item.label}/>)}</MenuGroup>)}</>; }

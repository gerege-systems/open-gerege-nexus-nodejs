...
function play(audio) { new Audio(`data:${audio.mime};base64,${audio.data}`).play().catch(() => undefined); }
export default function AICopilot() {
    const { t, locale } = useI18n();
    const [open, setOpen] = useState(false), [prompt, setPrompt] = useState(""), [loading, setLoading] = useState(false);
    const [mode, setMode] = useState("chat"), [target, setTarget] = useState("en"), [recording, setRecording] = useState(false);
    const [messages, setMessages] = useState([{ role: "model", text: t("ai.message.greeting") }]);
    const recorder = useRef(null), chunks = useRef([]), end = useRef(null);
    useEffect(() => { end.current?.scrollIntoView({ behavior: "smooth" }); }, [messages, loading]);
    async function dispatch(text, audio) {
        const value = (text ?? prompt).trim();
        if ((!value && !audio) || loading)
            return;
        const shown = value || t("ai.message.voice_message");
        const prior = messages.filter(m => !m.degraded).slice(-20).map(({ role, text }) => ({ role, text }));
        ...
        try {
            ...
            setMessages(m => [...m, { role: "model", text: e instanceof Error ? e.message : "AI request failed", degraded: true }]);
        }
        finally {
            setLoading(false);
        }
    ...
    async function toggleRecord() {
        ...
        try {
            ...
            rec.start();
            setRecording(true);
            setTimeout(() => { if (rec.state === "recording")
                rec.stop(); }, 30000);
        }
        catch {
            setMessages(m => [...m, { role: "model", text: t("ai.message.microphone_denied"), degraded: true }]);
        }
    }
    async function speak(text) { try {
        play(await api.speakAI(text.slice(0, 2000)));
    }
    catch { /* response remains readable */ } }
    return <>

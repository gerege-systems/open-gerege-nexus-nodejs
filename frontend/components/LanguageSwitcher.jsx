...
export default function LanguageSwitcher({ variant = "light" }) {
    ...
    return (<div className={`inline-flex items-center gap-0.5 rounded-lg border p-0.5 ${base}`} role="group" aria-label={t("base.field.language")}>
      {offered.map((option) => (<button key={option.code} type="button" onClick={() => setLocale(option.code)} aria-pressed={locale === option.code} title={option.label} className={`flex items-center gap-1.5 rounded-md px-2 py-1 text-xs font-semibold transition ${locale === option.code ? activeStyle : idleStyle}`}>

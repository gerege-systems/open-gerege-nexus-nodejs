 ...
 */
export const DEFAULT_LOCALES = ["mn", "en"];
export const OPTIONAL_LOCALES = LOCALES.map((l) => l.code).filter((code) => !DEFAULT_LOCALES.includes(code));
...
export function I18nProvider({ children }) {
    ...
    const value = useMemo(() => {
        const availableLocales = ALL_CODES.filter((code) => DEFAULT_LOCALES.includes(code) || extraLocales.includes(code));
        const setLocale = (next) => {
            window.localStorage.setItem(STORAGE_KEY, next);
            setLocaleState(next);
        };
        const setLocaleEnabled = (target, enabled) => {
            if (DEFAULT_LOCALES.includes(target))
                return; // mn/en are not switchable
            const next = enabled
        ...
        const t = (key, vars) => {
            ...
            let text = overlays[locale]?.[key] || (entry ? entry[locale] || entry.en : key);
            if (vars) {

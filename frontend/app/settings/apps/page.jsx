...
export default function InstalledAppsSettingsPage() {
    ...
    return (<div className="space-y-6">
      ...
      {loading ? (<div className="py-8 text-slate-500 text-sm">{t("app_store.message.loading_installed")}</div>) : apps.length === 0 ? (<div className="bg-white border border-slate-200 rounded-xl p-8 text-center text-slate-500 text-sm">
          ...
          <table className="w-full text-left border-collapse text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {apps.map((app) => (<tr key={app.id} className="hover:bg-slate-50">
                  ...
                  <td className="py-3.5 px-4">
                    <span className={`inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${app.enabled
                  ...
                  <td className="py-3.5 px-4 text-right">
                    <button onClick={() => handleToggle(app)} disabled={actionLoading === app.slug} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition border ${app.enabled

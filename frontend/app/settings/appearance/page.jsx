...
import { DEFAULT_LOCALES, LOCALES, useI18n } from "@/lib/i18n";
    ...
    return (<div className="w-full space-y-6">
      ...
      <section className="bg-white border border-slate-200 rounded-[var(--gerege-radius-card)] p-5 shadow-sm">
        ...
        <ul className="divide-y divide-slate-100 border border-slate-200 rounded-xl overflow-hidden">
          {LOCALES.map((option) => {
            ...
            return (<li key={option.code} className="flex items-center gap-3 px-4 py-3">
                ...
                <span className="text-xs text-slate-400">{t("appearance.state.language_always")}</span>) : (<button type="button" role="switch" aria-checked={isOn} aria-label={option.label} onClick={() => setLocaleEnabled(option.code, !isOn)} className={`relative w-11 h-6 rounded-full transition ${isOn ? "bg-[var(--gerege-blue)]" : "bg-slate-200"}`}>
      ...
      <section className="bg-white border border-slate-200 rounded-[var(--gerege-radius-card)] p-5 shadow-sm">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {styles.map((item) => (<button key={item.value} onClick={() => theme.updateTheme({ design: item.value })} className={`overflow-hidden rounded-xl border text-left transition ${theme.design === item.value ? "border-[var(--gerege-blue)] ring-2 ring-[color-mix(in_srgb,var(--gerege-blue)_15%,transparent)]" : "border-slate-200 hover:border-slate-300"}`}>
      ...
      <section className="bg-white border border-slate-200 rounded-[var(--gerege-radius-card)] p-5 shadow-sm">
        ...
        <div className="grid sm:grid-cols-3 gap-3">
          {modes.map(({ value, label, icon: Icon }) => (<button key={value} onClick={() => theme.updateTheme({ mode: value })} className={`relative flex items-center gap-3 p-4 rounded-lg border text-left transition ${theme.mode === value ? "border-[var(--gerege-blue)] bg-[var(--gerege-blue-soft)]" : "border-slate-200 hover:border-slate-300"}`}>
      ...
      <section className="bg-white border border-slate-200 rounded-[var(--gerege-radius-card)] p-5 shadow-sm">
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          {accents.map((accent) => (<button key={accent.value} onClick={() => theme.updateTheme({ accent: accent.value })} className={`flex items-center gap-3 p-3 rounded-lg border text-left ${theme.accent === accent.value ? "border-[var(--gerege-blue)] bg-[var(--gerege-blue-soft)]" : "border-slate-200 hover:border-slate-300"}`}>
      ...
      <section className="bg-white border border-slate-200 rounded-[var(--gerege-radius-card)] p-5 shadow-sm">
        ...
        <div className="mt-4 inline-flex rounded-lg border border-slate-200 p-1 bg-slate-50">
          {densities.map(({ value, label }) => (<button key={value} onClick={() => theme.updateTheme({ density: value })} className={`px-4 py-2 rounded-md text-sm font-medium ${theme.density === value ? "bg-white text-[var(--gerege-blue)] shadow-sm" : "text-slate-500"}`}>
              {t(label)}
            </button>))}
        </div>
      </section>

...
import { Activity, AlertTriangle, CheckCircle2, Cloud, Globe, HardDrive, Link2, Plus, RefreshCw, Share2, ShieldAlert, Trash2, Unlink, Video, } from "lucide-react";
/**
...
const OAUTH_PROVIDERS = ["google_drive", "dropbox", "google_meet"];
const emptyForm = {
    provider: "webhook",
    name: "",
    target_url: "",
...
export default function IntegrationsPage() {
    ...
    const providerInfo = (provider) => providers.find((p) => p.provider === provider);
    ...
    async function handleCreate(e) {
        ...
        try {
            const config = {};
            if (form.provider === "google_drive" && form.folder)
                config.folder_id = form.folder;
            if (form.provider === "dropbox" && form.folder)
                config.folder_path = form.folder;
            if (form.provider === "google_meet" && form.calendar_id)
                config.calendar_id = form.calendar_id;
    ...
    async function handleDelete(item) {
        if (!window.confirm(t("integrations.message.confirm_delete", { name: item.name })))
            return;
        setBusy(item.id);
        try {
    ...
    return (<div className="space-y-6">
      ...
      {!encryptionReady && (<div className="p-4 text-sm rounded-lg border bg-amber-50 border-amber-200 text-amber-800 flex items-start gap-2">
          <AlertTriangle className="w-4 h-4 mt-0.5 shrink-0"/>
          <span>{t("integrations.message.encryption_missing")}</span>
        </div>)}

      {loading ? (<div className="py-12 text-center text-slate-400">{t("integrations.message.loading")}</div>) : integrations.length === 0 ? (<div className="bg-white border border-slate-200 rounded-xl p-12 text-center text-slate-500 text-sm">
          ...
          {integrations.map((item) => {
                ...
                return (<div key={item.id} className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between gap-3">
                <div>
                  <div className="flex items-start justify-between mb-3 gap-3">
                    ...
                    <span className={`inline-flex items-center space-x-1 text-xs font-bold px-2.5 py-1 rounded-full shrink-0 ${health === "ok"
                        ? "bg-emerald-50 text-emerald-700 border border-emerald-200"
                        : health === "failing"
                            ? "bg-red-50 text-red-700 border border-red-200"
                            : "bg-amber-50 text-amber-700 border border-amber-200"}`}>
                      {health === "ok" ? (<CheckCircle2 className="w-3.5 h-3.5"/>) : health === "failing" ? (<AlertTriangle className="w-3.5 h-3.5"/>) : (<ShieldAlert className="w-3.5 h-3.5"/>)}
                  ...
                  <div className="text-xs text-slate-500 space-y-1 bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                    {oauth ? (<div className="truncate">
                        {item.connected
                            ? t("integrations.field.connected_account") + ": " + (item.account_label || "—")
                            : t("integrations.message.not_connected")}
                      </div>) : (<div className="truncate font-mono">{item.target_url}</div>)}
                    {item.config?.auto_export === "true" && (<div className="text-emerald-700">{t("integrations.message.auto_export_on")}</div>)}
                    {item.last_ping_at && (<div className="flex items-center gap-1">
                ...
                <div className="flex items-center gap-2">
                  {oauth &&
                        (item.connected ? (<button onClick={() => void handleDisconnect(item)} disabled={busy === item.id} className="flex-1 flex items-center justify-center gap-1.5 border border-slate-300 text-slate-700 hover:bg-slate-50 text-xs font-semibold py-2 rounded-lg disabled:opacity-50">
                        <Unlink className="w-3.5 h-3.5"/>
                        {t("integrations.action.disconnect")}
                      </button>) : (<button onClick={() => void handleConnect(item)} disabled={busy === item.id} className="flex-1 flex items-center justify-center gap-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold py-2 rounded-lg disabled:opacity-50">
                        <Link2 className="w-3.5 h-3.5"/>
                        {t("integrations.action.connect")}
                      </button>))}
                  <button onClick={() => void handleDelete(item)} disabled={busy === item.id} aria-label={t("base.action.delete")} className="p-2 border border-slate-300 text-red-600 hover:bg-red-50 rounded-lg disabled:opacity-50">
          ...
          <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-xl border border-slate-200 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              <div>
                ...
                <select value={form.provider} onChange={(e) => setForm({ ...form, provider: e.target.value })} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                  {Object.keys(PROVIDER_LABEL_KEYS).map((provider) => {
                const info = providerInfo(provider);
                return (<option key={provider} value={provider} disabled={info ? !info.available : false}>
                        {t(PROVIDER_LABEL_KEYS[provider])}
                        {info && !info.available ? " — " + t("integrations.state.unavailable") : ""}
                      </option>);
            })}
                </select>
                {selected && !selected.available && (<p className="mt-1 text-[11px] text-amber-700">{selected.reason}</p>)}
              </div>
              ...
              <div>
                ...
                <input type="text" placeholder={t("integrations.field.name_placeholder")} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500" required/>
              </div>
              ...
              {!isOAuth && (<>
                  <div>
                    ...
                    <input type="url" placeholder="https://api.example.com/webhooks" value={form.target_url} onChange={(e) => setForm({ ...form, target_url: e.target.value })} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500" required/>
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 mb-1">
                      {t("integrations.field.secret")}
                    </label>
                    <input type="password" placeholder={t("integrations.field.secret_placeholder")} value={form.secret_key} onChange={(e) => setForm({ ...form, secret_key: e.target.value })} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500"/>
                  </div>
                </>)}
              ...
              {(form.provider === "google_drive" || form.provider === "dropbox") && (<>
                  <div>
                    ...
                    <input type="text" placeholder={form.provider === "google_drive"
                    ? t("integrations.field.drive_folder_placeholder")
                    : t("integrations.field.dropbox_folder_placeholder")} value={form.folder} onChange={(e) => setForm({ ...form, folder: e.target.value })} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500"/>
                  </div>
                  <label className="flex items-start gap-2 text-xs text-slate-700">
                    <input type="checkbox" checked={form.auto_export} onChange={(e) => setForm({ ...form, auto_export: e.target.checked })} className="mt-0.5"/>
                    <span>{t("integrations.field.auto_export")}</span>
                  </label>
                </>)}

              {form.provider === "google_meet" && (<div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">
                    {t("integrations.field.calendar_id")}
                  </label>
                  <input type="text" placeholder="primary" value={form.calendar_id} onChange={(e) => setForm({ ...form, calendar_id: e.target.value })} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500"/>
              ...
              <div className="flex items-center space-x-2 pt-2">
                <button type="button" onClick={() => setShowModal(false)} className="w-1/2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium py-2 rounded-lg text-xs">
                  {t("base.action.cancel")}
                </button>
                <button type="submit" className="w-1/2 bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 rounded-lg text-xs">

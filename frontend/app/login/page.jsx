...
export default function LoginPage() {
    ...
    const { t } = useI18n();
    const [next, setNext] = useState("/apps"), [admin, setAdmin] = useState(false), [email, setEmail] = useState("admin@example.com"), [password, setPassword] = useState("Password123!"), [error, setError] = useState("");
    useEffect(() => { const requested = new URLSearchParams(location.search).get("next"); if (requested?.startsWith("/") && !requested.startsWith("//"))
        setNext(requested); }, []);
    ...
    async function passwordLogin(e) { e.preventDefault(); setError(""); try {

...
export default function BillingPage() {
    ...
    return (<div className="space-y-6">
      ...
      {loading ? (<div className="py-12 text-center text-slate-400">{t("billing.message.loading")}</div>) : (<div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <table className="w-full text-left text-xs text-slate-600">
            ...
            <tbody className="divide-y divide-slate-100">
              {invoices.map((inv) => (<tr key={inv.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center space-x-1 text-[11px] font-bold px-2.5 py-0.5 rounded-full ${inv.status === "PAID"
          ...
          <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-xl border border-slate-200">
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              <div>
                ...
                <input type="text" placeholder={t("billing.field.contact_placeholder")} value={form.contact_name} onChange={(e) => setForm({ ...form, contact_name: e.target.value })} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500" required/>
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">{t("billing.field.amount")} *</label>
                <input type="number" placeholder="150000" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500" required/>
              </div>

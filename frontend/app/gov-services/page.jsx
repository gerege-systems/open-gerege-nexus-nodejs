...
export default function GovServicesPage() {
    ...
    return (<div className="space-y-6">
      <PageHeader icon={<Landmark className="w-7 h-7 text-[var(--gerege-blue)]"/>} title={t("gov.view.title")} subtitle={t("gov.view.subtitle")}/>

      <TaskQueue />

      <section className="grid sm:grid-cols-2 xl:grid-cols-3 gap-3">
        {SHORTCUTS.map(({ id, href, Icon }) => (<Link key={id} href={href} className="group p-4 bg-white border border-slate-200 rounded-xl hover:border-[var(--gerege-blue)] transition-colors">
            <div className="flex items-start gap-3">
              <Icon className="w-5 h-5 text-[var(--gerege-blue)] shrink-0 mt-0.5"/>
              <div className="flex-1 min-w-0">
                <div className="font-semibold text-slate-800">{t(`gov.menu.${id}`)}</div>
                <p className="text-xs text-slate-500 mt-1 leading-snug">{t(`gov.view.${id}_hint`)}</p>
              </div>

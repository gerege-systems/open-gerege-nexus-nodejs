...
import { Banner, PENDING, RowActions, SignatureCell, SignatureDialog, SignatureHistoryButton, SignatureHistoryDialog, SignatureProgress, StatusBadge, useDocumentActions, } from "@/components/documents/shared";
...
export default function DocumentsPage() {
    ...
    const loadSpan = async (rows, asked) => {
    ...
    const commitRename = async () => {
        ...
        setRenaming(null);
        if (!row || title.trim() === "" || title.trim() === row.title)
            return;
        setMessage(null);
        try {
            const saved = (await api.renameDocument(id, title.trim()));
            // Only this row, with what the server stored.
            if (saved?.id)
                setDocuments((current) => current.map((d) => (d.id === id ? { ...d, ...saved } : d)));
    ...
    return (<div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {can("documents.manage") && (<button onClick={() => setShowModal(true)} className="bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold px-4 py-2 rounded-lg flex items-center space-x-2 shadow-sm transition">
      ...
      {loadFailed && documents.length === 0 && (<div className="flex items-center justify-between gap-3 px-4 py-3 border border-amber-200 rounded-xl bg-amber-50">
      ...
      <section className="flex flex-wrap items-center gap-2">
        <input type="text" value={search} placeholder={t("documents.field.search_placeholder")} maxLength={255} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => {
            if (e.key === "Enter")
                loadSpan(PAGE, filterRef.current);
        }} onBlur={() => {
            ...
            if (search.trim() !== loadedFilter.current.q)
                loadSpan(PAGE, filterRef.current);
      ...
      {loading && documents.length === 0 ? (<div className="py-12 text-center text-slate-400">{t("documents.message.loading")}</div>) : documents.length === 0 ? (
        ...
        loadFailed ? null : (<div className="bg-white border border-slate-200 rounded-xl p-8 text-center text-slate-500 text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {documents.map((doc) => (<tr key={doc.id} className="hover:bg-slate-50">
                  <td className="px-4 py-3 font-semibold text-slate-900">
                    {renaming?.id === doc.id ? (<input autoFocus value={renaming.title} maxLength={255} onChange={(e) => setRenaming({ id: doc.id, title: e.target.value })} onBlur={commitRename} onKeyDown={(e) => {
                        if (e.key === "Enter")
                            commitRename();
                        if (e.key === "Escape")
                            setRenaming(null);
                    }} className="w-full px-2 py-1 text-xs font-semibold border border-indigo-300 rounded focus:ring-2 focus:ring-indigo-500"/>) : (<span className="flex items-center gap-1.5">
                        <span>{doc.title}</span>
                        ...
                        (doc.status === PENDING || doc.status === "DRAFT") && (<button type="button" title={t("documents.action.rename")} aria-label={t("documents.action.rename")} onClick={() => setRenaming({ id: doc.id, title: doc.title })} className="shrink-0 text-slate-300 hover:text-indigo-600">
                  ...
                  <td className="px-4 py-3 text-right">
                    <RowActions doc={doc} busy={isBusy(doc.id)} canSign={can("documents.sign")} canManage={can("documents.manage")} onSign={setSignTarget} onReject={reject} onRoute={route}/>
                  </td>
                </tr>))}
            </tbody>
          </table>
          ...
          {hasMore && (<div className="flex items-center justify-between gap-3 px-4 py-3 border-t border-slate-200 bg-slate-50">
              <p className="text-[11px] text-slate-500">
                {t("documents.message.showing_some", { shown: documents.length, total })}
              </p>
              <button type="button" disabled={loading} onClick={loadMore} className="text-[11px] font-semibold px-3 py-1.5 rounded-lg bg-white border border-slate-300 text-slate-700 hover:bg-slate-100 disabled:opacity-50">
      ...
      {showModal && (<div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-xl border border-slate-200">
            ...
            {createFailure && (<p className="mb-4 text-xs text-red-700 bg-red-50 border border-red-200 rounded-lg p-3">
                {createFailure}
              </p>)}
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                ...
                <input type="text" placeholder={t("documents.field.title_placeholder")} value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500" maxLength={255} required/>
              </div>
              ...
              <div className="flex items-center space-x-2 pt-2">
                ...
                <button type="submit" disabled={creating || !form.title.trim()} className="w-1/2 bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 rounded-lg text-xs disabled:opacity-50">{t("documents.action.create")}</button>
              </div>
            </form>
          </div>
        </div>)}

      {signTarget && (<SignatureDialog key={signTarget.id} doc={signTarget} onClose={() => setSignTarget(null)} onDone={async (text) => {
                setSignTarget(null);
                await succeed(text);
            }} onError={fail}/>)}

      {historyTarget && (<SignatureHistoryDialog doc={historyTarget} onClose={() => setHistoryTarget(null)}/>)}
    </div>);
}

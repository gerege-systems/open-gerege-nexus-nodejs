...
import { Banner, PENDING, RowActions, SectionHeader, SignatureDialog, SignatureHistoryButton, SignatureHistoryDialog, SignatureProgress, useDocumentActions, } from "@/components/documents/shared";
...
export default function DocumentApprovalsPage() {
    ...
    return (<div className="space-y-6">
      <SectionHeader icon={<ListChecks className="w-7 h-7 text-indigo-600"/>} title={t("documents.menu.approvals")} subtitle={t("documents.view.approvals_hint")}/>
      ...
      {loadFailed && pending.length === 0 && (<div className="flex items-center justify-between gap-3 px-4 py-3 border border-amber-200 rounded-xl bg-amber-50">
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        {byType.slice(0, 2).map(([docType, count]) => (<div key={docType} className="p-4 bg-white border border-slate-200 rounded-xl">
      ...
      {loading && pending.length === 0 ? (<div className="py-12 text-center text-slate-400">{t("documents.message.loading")}</div>) : pending.length === 0 ? (
        ...
        loadFailed ? null : (<div className="bg-white border border-slate-200 rounded-xl p-8 text-center text-slate-500 text-sm">
          ...
          <table className="w-full text-left text-xs text-slate-600">
            ...
            <tbody className="divide-y divide-slate-100">
              {pending.map((doc) => (<tr key={doc.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-right">
                    <RowActions doc={doc} busy={isBusy(doc.id)} canSign={can("documents.sign")} onSign={setSignTarget} onReject={reject}/>
                  </td>
                </tr>))}
            </tbody>
          </table>
      ...
      {ready && !can("documents.sign") && pending.length > 0 && (<p className="text-xs text-slate-500">{t("documents.message.sign_not_granted")}</p>)}

...
export default function DocumentRetentionPage() {
    ...
    const edit = (docType, patch) => setRules((current) => current.map((r) => (r.doc_type === docType ? { ...r, ...patch } : r)));
    ...
    const atLeast = (value, partial) => loadFailed ? "—" : partial ? `≥${value}` : `${value}`;
    return (<div className="space-y-6">
      <SectionHeader icon={<Archive className="w-7 h-7 text-indigo-600"/>} title={t("documents.menu.retention")} subtitle={t("documents.view.retention_hint")}/>
      ...
      {loading ? (<div className="py-12 text-center text-slate-400">{t("documents.message.loading")}</div>) : loadFailed ? (
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-8 text-center text-slate-500 text-sm">
          ...
          <table className="w-full text-left text-xs text-slate-600">
            ...
            <tbody className="divide-y divide-slate-100">
              {rules.map((rule) => (<tr key={rule.doc_type} className="hover:bg-slate-50">
                  <td className="px-4 py-3 font-mono font-semibold text-slate-900">{rule.doc_type}</td>
                  <td className="px-4 py-3">
                    <input type="number" min={1} max={100} value={rule.retain_years || ""} disabled={!mayManage} onChange={(e) => edit(rule.doc_type, { retain_years: Number(e.target.value) })} className="w-20 px-2 py-1.5 text-xs border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500"/>
                  </td>
                  <td className="px-4 py-3">
                    <input type="text" value={rule.note} disabled={!mayManage} onChange={(e) => edit(rule.doc_type, { note: e.target.value })} className="w-full px-2 py-1.5 text-xs border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500"/>
                  </td>
                  <td className="px-4 py-3 text-slate-500">{rule.total ?? "—"}</td>
                  <td className="px-4 py-3">
                    {rule.expired === undefined ? (<span className="text-slate-400">—</span>) : rule.expired > 0 ? (<span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 border border-amber-200">
                        {rule.expired}
                      </span>) : (<span className="text-slate-400">0</span>)}
                  </td>
                  <td className="px-4 py-3 text-right">
                    {mayManage ? (<button onClick={() => save(rule)} disabled={busy === rule.doc_type || !rule.retain_years} className="inline-flex items-center space-x-1 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold border border-indigo-200 text-indigo-600 hover:bg-indigo-50 disabled:opacity-50">

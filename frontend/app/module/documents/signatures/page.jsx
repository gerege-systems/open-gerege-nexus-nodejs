...
export default function SignaturePoliciesPage() {
    ...
    const edit = (docType, patch) => setPolicies((current) => current.map((p) => (p.doc_type === docType ? { ...p, ...patch } : p)));
    ...
    return (<div className="space-y-6">
      <SectionHeader icon={<PenTool className="w-7 h-7 text-indigo-600"/>} title={t("documents.menu.signature_policies")} subtitle={t("documents.view.signature_policies_hint")}/>
      ...
      {loading ? (<div className="py-12 text-center text-slate-400">{t("documents.message.loading")}</div>) : loadFailed ? (<div className="bg-white border border-slate-200 rounded-xl p-8 text-center text-slate-500 text-sm">
          ...
          <table className="w-full text-left text-xs text-slate-600">
            ...
            <tbody className="divide-y divide-slate-100">
              {policies.map((policy) => (<tr key={policy.doc_type} className="hover:bg-slate-50">
                  <td className="px-4 py-3 font-mono font-semibold text-slate-900">{policy.doc_type}</td>
                  <td className="px-4 py-3">
                    <input type="checkbox" checked={policy.allow_eid} disabled={!mayManage} onChange={(e) => edit(policy.doc_type, { allow_eid: e.target.checked })} className="w-4 h-4 accent-indigo-600"/>
                  </td>
                  <td className="px-4 py-3">
                    <input type="checkbox" checked={policy.allow_dan} disabled={!mayManage} onChange={(e) => edit(policy.doc_type, { allow_dan: e.target.checked })} className="w-4 h-4 accent-indigo-600"/>
                  </td>
                  <td className="px-4 py-3">
                    <input type="checkbox" checked={policy.require_named_signer} disabled={!mayManage} onChange={(e) => edit(policy.doc_type, { require_named_signer: e.target.checked })} className="w-4 h-4 accent-indigo-600"/>
                  </td>
                  <td className="px-4 py-3">
                    {policy.configured ? (<span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-200">
                        {t("documents.state.policy_configured")}
                      </span>) : (<span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 border border-slate-200">
                        {t("documents.state.policy_default")}
                      </span>)}
                  </td>
                  <td className="px-4 py-3 text-right">
                    {mayManage ? (<button onClick={() => save(policy)} disabled={busy === policy.doc_type || (!policy.allow_eid && !policy.allow_dan)} className="inline-flex items-center space-x-1 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold border border-indigo-200 text-indigo-600 hover:bg-indigo-50 disabled:opacity-50">
                        <Save className="w-3.5 h-3.5"/>
                        <span>{t("base.action.save")}</span>
                      </button>) : (<span className="text-slate-300 text-[11px]">—</span>)}
                  </td>
                </tr>))}
            </tbody>
          </table>

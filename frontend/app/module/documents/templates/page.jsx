...
import { Banner, SectionHeader } from "@/components/documents/shared";
import { Files, Plus, Save, Trash2, Wand2 } from "lucide-react";
const DOC_TYPES = ["CONTRACT", "REQUEST", "APPROVAL"];
/**
...
export default function DocumentTemplatesPage() {
    ...
    const [busy, setBusyIds] = useState(new Set());
    const isBusy = (id) => busy.has(id);
    const setBusy = (id, working) => setBusyIds((current) => {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            setTemplates((current) => {
                ...
                const kept = live.map((row) => dirtyRef.current[row.id] ? current.find((row2) => row2.id === row.id) ?? row : row);
    ...
    const handleCreate = async (e) => {
        ...
        setMessage(null);
        try {
            const created = (await api.createDocumentTemplate(form));
    ...
    const edit = (id, patch, saved = false) => {
    ...
    const reconcile = (tpl, err) => {
        if (err?.status === 404) {
            removedRef.current.add(tpl.id);
            setTemplates((current) => current.filter((row) => row.id !== tpl.id));
            return;
        }
        if (err?.status === 409 && tpl.active)
            edit(tpl.id, { active: false }, true);
    };
    const handleUse = async (tpl) => {
        setBusy(tpl.id, true);
        setMessage(null);
        try {
            const doc = await api.useDocumentTemplate(tpl.id);
            setMessage({
                type: "success",
    ...
    const handleDelete = async (tpl) => {
        if (!confirm(t("documents.message.template_delete_confirm", { name: tpl.name })))
            return;
        setBusy(tpl.id, true);
        setMessage(null);
        try {
    ...
    return (<div className="space-y-6">
      <SectionHeader icon={<Files className="w-7 h-7 text-indigo-600"/>} title={t("documents.menu.templates")} subtitle={t("documents.view.templates_hint")}/>

      {message && <Banner message={message} onDismiss={() => setMessage(null)}/>}

      {mayManage && (<form onSubmit={handleCreate} className="bg-white border border-slate-200 rounded-xl p-4 grid gap-3 md:grid-cols-4">
          <div className="md:col-span-1">
            <label className="block text-xs font-semibold text-slate-700 mb-1">{t("documents.field.template_name")}</label>
            <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500" required/>
          </div>
          <div className="md:col-span-1">
            <label className="block text-xs font-semibold text-slate-700 mb-1">{t("documents.field.category")}</label>
            <select value={form.doc_type} onChange={(e) => setForm({ ...form, doc_type: e.target.value })} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
          ...
          <div className="md:col-span-1">
            ...
            <input type="text" placeholder={t("documents.field.title_pattern_placeholder")} value={form.title_pattern} onChange={(e) => setForm({ ...form, title_pattern: e.target.value })} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500" required/>
          </div>
          <div className="md:col-span-1 flex items-end">
            <button type="submit" disabled={isBusy("create") || !form.name.trim() || !form.title_pattern.trim()} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold px-4 py-2 rounded-lg flex items-center justify-center space-x-2 disabled:opacity-50">
      ...
      {loading ? (<div className="py-12 text-center text-slate-400">{t("documents.message.loading")}</div>) : templates.length === 0 ? (
        ...
        loadFailed ? null : (<div className="bg-white border border-slate-200 rounded-xl p-8 text-center text-slate-500 text-sm">
          ...
          <table className="w-full text-left text-xs text-slate-600">
            ...
            <tbody className="divide-y divide-slate-100">
              {templates.map((tpl) => (<tr key={tpl.id} className={`hover:bg-slate-50 ${tpl.active ? "" : "opacity-60"}`}>
                  <td className="px-4 py-3">
                    <input type="text" value={tpl.name} disabled={!mayManage} onChange={(e) => edit(tpl.id, { name: e.target.value })} className="w-full px-2 py-1.5 text-xs font-semibold border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 disabled:border-transparent disabled:bg-transparent"/>
                  </td>
                  <td className="px-4 py-3">
                    <select value={tpl.doc_type} disabled={!mayManage} onChange={(e) => edit(tpl.id, { doc_type: e.target.value })} className="px-2 py-1.5 text-xs font-mono border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 disabled:border-transparent disabled:bg-transparent">
                      {DOC_TYPES.map((type) => (<option key={type} value={type}>
                          {type}
                        </option>))}
                    </select>
                  ...
                  <td className="px-4 py-3">
                    <input type="text" value={tpl.title_pattern} disabled={!mayManage} onChange={(e) => edit(tpl.id, { title_pattern: e.target.value })} className="w-full px-2 py-1.5 text-xs font-mono border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 disabled:border-transparent disabled:bg-transparent"/>
                  </td>
                  <td className="px-4 py-3">
                    <input type="checkbox" checked={tpl.active} disabled={!mayManage} onChange={(e) => edit(tpl.id, { active: e.target.checked })} title={t("documents.message.template_active_hint")} className="w-4 h-4 accent-indigo-600"/>
                  </td>
                  <td className="px-4 py-3 text-right">
                    {mayManage ? (<div className="flex items-center justify-end space-x-2">
                        <button onClick={() => handleSave(tpl)} disabled={isBusy(tpl.id) || !tpl.name.trim() || !tpl.title_pattern.trim()} className="inline-flex items-center space-x-1 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold border border-slate-300 text-slate-700 hover:bg-slate-50 disabled:opacity-50">
                          <Save className="w-3.5 h-3.5"/>
                          <span>{t("base.action.save")}</span>
                        </button>
                        <button onClick={() => handleUse(tpl)} disabled={isBusy(tpl.id) || !tpl.active || dirty[tpl.id]} title={dirty[tpl.id]
                        ? t("documents.message.template_unsaved")
                        : tpl.active
                            ? undefined
                            : t("documents.message.template_inactive")} className="inline-flex items-center space-x-1 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold border border-indigo-200 text-indigo-600 hover:bg-indigo-50 disabled:opacity-50">
                          <Wand2 className="w-3.5 h-3.5"/>
                          <span>{t("documents.action.use_template")}</span>
                        </button>
                        <button onClick={() => handleDelete(tpl)} disabled={isBusy(tpl.id)} className="inline-flex items-center space-x-1 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold border border-red-200 text-red-600 hover:bg-red-50 disabled:opacity-50">
                          <Trash2 className="w-3.5 h-3.5"/>
                          <span>{t("base.action.delete")}</span>
                        </button>
                      </div>) : (<span className="text-slate-300 text-[11px]">—</span>)}
                  </td>
                </tr>))}
            </tbody>
          </table>
          ...
          {loadFailed && (<div className="flex items-center justify-between gap-3 px-4 py-3 border-t border-amber-200 bg-amber-50">
              <p className="text-[11px] text-amber-800">{t("documents.message.stale_rows")}</p>
              <button type="button" disabled={loading} onClick={() => loadData()} className="text-[11px] font-semibold px-3 py-1.5 rounded-lg bg-white border border-amber-300 text-amber-800 hover:bg-amber-100 disabled:opacity-50">

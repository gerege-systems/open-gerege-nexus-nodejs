...
export default function GovAppointmentsPage() {
    ...
    const [form, setForm] = useState({ service_id: "", citizen_name: "", scheduled_at: "", location: "", mode: "IN_PERSON" });
    ...
    if (loading)
        return <Loading />;
    const format = (value) => new Date(value).toLocaleString(locale === "en" ? "en-GB" : "mn-MN");
    return (<div className="space-y-6">
      <PageHeader icon={<CalendarClock className="w-7 h-7 text-[var(--gerege-blue)]"/>} title={t("gov.menu.appointments")} subtitle={t("gov.view.appointments_hint")}/>
      ...
      <section className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
        ...
        <form onSubmit={book} className="grid sm:grid-cols-6 gap-2">
          <select required value={form.service_id} onChange={(e) => setForm({ ...form, service_id: e.target.value })} className="px-3 py-2 text-sm border border-slate-300 rounded-lg bg-white">
          ...
          <input required placeholder={t("gov.field.citizen")} value={form.citizen_name} onChange={(e) => setForm({ ...form, citizen_name: e.target.value })} className="px-3 py-2 text-sm border border-slate-300 rounded-lg"/>
          <input required type="datetime-local" value={form.scheduled_at} onChange={(e) => setForm({ ...form, scheduled_at: e.target.value })} className="px-3 py-2 text-sm border border-slate-300 rounded-lg"/>
          <select value={form.mode} aria-label={t("gov.field.appointment_mode")} onChange={(e) => setForm({ ...form, mode: e.target.value })} className="px-3 py-2 text-sm border border-slate-300 rounded-lg">
          ...
          <input placeholder={t("gov.field.location")} value={form.location} disabled={form.mode === "ONLINE"} onChange={(e) => setForm({ ...form, location: e.target.value })} className="px-3 py-2 text-sm border border-slate-300 rounded-lg disabled:bg-slate-100 disabled:text-slate-400"/>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        {appointments.length === 0 ? (<EmptyState message={t("gov.message.no_appointments")}/>) : (<div className="overflow-x-auto">
              ...
              <tbody className="divide-y divide-slate-100">
                {appointments.map((item) => (<tr key={item.id}>
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded ${item.mode === "ONLINE" ? "bg-violet-100 text-violet-700" : "bg-slate-100 text-slate-600"}`}>
                        {item.mode === "ONLINE" ? t("gov.state.online") : t("gov.state.in_person")}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-slate-500">
                      {item.mode === "ONLINE" ? (item.meeting_url ? (<a href={item.meeting_url} target="_blank" rel="noreferrer noopener" className="text-[var(--gerege-blue)] font-medium underline">

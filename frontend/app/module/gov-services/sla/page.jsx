...
import { Banner, DashboardCards, EmptyState, Loading, PageHeader, StatusBadge, describeError, } from "@/components/gov/shared";
...
export default function GovSlaPage() {
    ...
    const load = useCallback(async () => {
        try {
            ...
            const published = workflows.flatMap((wf) => (wf.versions || []).filter((v) => v.status === "PUBLISHED").map((v) => ({ workflow: wf, version: v })));
            const detailed = await Promise.all(published.map(async (entry) => ({
    ...
    return (<div className="space-y-6">
      <PageHeader icon={<Timer className="w-7 h-7 text-[var(--gerege-blue)]"/>} title={t("gov.menu.sla")} subtitle={t("gov.view.sla_hint")}/>
      ...
      <section className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
        ...
        {versions.length === 0 ? (<p className="text-sm text-slate-500">{t("gov.message.no_published_workflow")}</p>) : (versions.map(({ workflow, version }) => (<div key={version.id} className="space-y-2">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm min-w-[560px]">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {(version.steps || []).map((step) => (<tr key={step.id}>
                        ...
                        <td className="px-3 py-2">
                          {step.requires_verification ? (<span className="text-xs font-semibold px-2 py-0.5 rounded bg-orange-100 text-orange-700">
                              {t("base.value.yes")}
                            </span>) : (<span className="text-xs text-slate-400">{t("base.value.no")}</span>)}
                        </td>
                      </tr>))}
                  </tbody>
                </table>
      ...
      <section className="space-y-2">
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
          {!overdue || overdue.items.length === 0 ? (<EmptyState message={t("gov.message.queue_empty")}/>) : (<div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[720px]">
                ...
                <tbody className="divide-y divide-slate-100">
                  {overdue.items.map((task) => (<tr key={task.id} className="bg-red-50/40">
      ...
      {detail && (<RequestDrawer detail={detail} onClose={() => setDetail(null)} unitName={(id) => id || "—"} statusBadge={(status) => <StatusBadge status={status}/>}/>)}
    </div>);
}

...
export default function EsignHSMPage() {
    ...
    return (<div className="space-y-6">
      <PageHeader icon={<ServerCog className="w-7 h-7 text-indigo-600"/>} title={t("esign.view.hsm_title")} subtitle={t("esign.view.hsm_subtitle")} actions={<button onClick={test} disabled={testing} className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white text-xs font-semibold px-4 py-2 rounded-lg flex items-center gap-2 shadow-sm">
      ...
      {!hsm.mock_mode && !hsm.has_token && (<Banner tone="error" message={t("esign.message.hsm_no_token")}/>)}
      ...
      <div className="grid lg:grid-cols-2 gap-6 items-start">
        <Card title={t("esign.view.hsm_connection")}>
          <dl className="divide-y divide-slate-100 text-sm">
            ...
            <Row label={t("esign.field.mode")} value={hsm.mock_mode ? (<span className="text-amber-700 font-semibold">{t("esign.state.mock")}</span>) : (<span className="text-emerald-700 font-semibold">{t("esign.state.live")}</span>)}/>
            <Row label={t("esign.field.token")} value={hsm.has_token ? (<span className="inline-flex items-center gap-1.5 text-emerald-700 font-semibold">
                    <CheckCircle2 className="w-4 h-4"/>
                    {t("esign.state.token_present")}
                  </span>) : (<span className="inline-flex items-center gap-1.5 text-slate-500">
        ...
        <Card title={t("esign.view.hsm_last_probe")}>
          {probe ? (<div className="p-4 space-y-3">
              <div className={`p-3 rounded-lg border text-sm flex items-start gap-2 ${probe.ok
                ? "bg-emerald-50 border-emerald-200 text-emerald-700"
                : "bg-red-50 border-red-200 text-red-700"}`}>
                {probe.ok ? (<CheckCircle2 className="w-4 h-4 mt-0.5 shrink-0"/>) : (<XCircle className="w-4 h-4 mt-0.5 shrink-0"/>)}
            ...
            </div>) : (<p className="p-6 text-sm text-slate-500 text-center italic">{t("esign.message.no_probe_yet")}</p>)}
        </Card>
      </div>
    </div>);
}
function Row({ label, value }) {
    return (<div className="px-4 py-3 flex items-start justify-between gap-4">

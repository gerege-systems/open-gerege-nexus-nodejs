...
import { Banner, EmptyState, Loading, OutcomeBadge, PageHeader, Pager, useErrorMessage, } from "@/components/esign/shared";
const PAGE_SIZE = 50;
/**
...
export default function EsignLogsPage() {
    ...
    const load = useCallback(async (nextOffset, nextFilter) => {
        setLoading(true);
        try {
    ...
    return (<div className="space-y-6">
      <PageHeader icon={<ScrollText className="w-7 h-7 text-indigo-600"/>} title={t("esign.view.logs_title")} subtitle={t("esign.view.logs_subtitle")} actions={<button onClick={exportCsv} className="bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold px-4 py-2 rounded-lg flex items-center gap-2">
      ...
      <section className="bg-white border border-slate-200 rounded-xl shadow-sm p-4 flex flex-wrap gap-3 items-end">
        <form onSubmit={submitSearch} className="flex-1 min-w-[220px]">
          ...
          <div className="relative">
            ...
            <input id="log-search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t("esign.field.search_placeholder")} className="w-full pl-9 pr-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500"/>
          </div>
        </form>

        <Select id="log-action" label={t("esign.field.action")} value={filter.action ?? ""} onChange={(value) => applyFilter({ action: value || undefined })} options={[
            ["", t("base.label.all")],
        ...
        <Select id="log-outcome" label={t("esign.field.outcome")} value={filter.outcome ?? ""} onChange={(value) => applyFilter({ outcome: value || undefined })} options={[
            ["", t("base.label.all")],
        ...
        <Select id="log-provider" label={t("esign.field.provider")} value={filter.provider ?? ""} onChange={(value) => applyFilter({ provider: value || undefined })} options={[
            ["", t("base.label.all")],
            ["EID", "eID Mongolia"],
        ...
        <div>
          ...
          <input id="log-from" type="date" value={filter.from ?? ""} onChange={(event) => applyFilter({ from: event.target.value || undefined })} className="px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500"/>
        </div>
        <div>
          <label htmlFor="log-to" className="block text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-1">
            {t("esign.field.to")}
          </label>
          <input id="log-to" type="date" value={filter.to ?? ""} onChange={(event) => applyFilter({ to: event.target.value || undefined })} className="px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500"/>
        </div>
      </section>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-x-auto">
        {loading ? (<div className="p-6">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {entries.map((entry) => (<tr key={entry.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">
                    {[entry.last_name, entry.first_name].filter(Boolean).join(" ") || entry.reg_no || (<span className="text-slate-400">—</span>)}
                    {entry.reg_no && (entry.first_name || entry.last_name) && (<div className="text-[10px] text-slate-400 font-mono">{entry.reg_no}</div>)}
                  </td>
      ...
      <Pager total={total} offset={offset} pageSize={PAGE_SIZE} onPage={(next) => load(next, filter)}/>
    </div>);
}
function Select({ id, label, value, onChange, options, }) {
    return (<div>
      ...
      <select id={id} value={value} onChange={(event) => onChange(event.target.value)} className="px-3 py-2 text-sm border border-slate-300 rounded-lg bg-white focus:ring-2 focus:ring-indigo-500">
        {options.map(([optionValue, optionLabel]) => (<option key={optionValue} value={optionValue}>
            {optionLabel}
          </option>))}
      </select>
    </div>);

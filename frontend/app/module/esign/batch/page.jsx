...
import { BatchBadge, Banner, Card, EmptyState, ItemBadge, Loading, PageHeader, useErrorMessage, } from "@/components/esign/shared";
/**
 * Batch signing.
 *
...
export default function EsignBatchPage() {
    ...
    if (selected) {
        return (<BatchDetail batch={selected} onBack={async () => {
                setSelected(null);
                await load();
            }} onRefresh={async (id) => setSelected(await esign.batch(id))}/>);
    }
    return (<div className="space-y-6">
      <PageHeader icon={<Layers className="w-7 h-7 text-indigo-600"/>} title={t("esign.view.batch_title")} subtitle={t("esign.view.batch_subtitle")} actions={<button onClick={() => setCreating(true)} className="bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold px-4 py-2 rounded-lg flex items-center gap-2 shadow-sm">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-x-auto">
        {loading ? (<div className="p-6">
            <Loading />
          </div>) : (<table className="w-full text-left text-xs text-slate-600">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {batches.map((batch) => (<tr key={batch.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => open(batch)} className="bg-slate-100 hover:bg-slate-200 text-slate-700 text-[11px] font-semibold px-3 py-1.5 rounded-lg">
                      {t("base.action.open")}
                    </button>
                  </td>
                </tr>))}
      ...
      {creating && (<CreateBatchModal onClose={() => setCreating(false)} onCreated={async (batch) => {
                setCreating(false);
                await load();
                setSelected(batch);
            }}/>)}
...
function Progress({ signed, failed, total }) {
...
function BatchDetail({ batch, onBack, onRefresh, }) {
    const { t } = useI18n();
    const describe = useErrorMessage();
    ...
    return (<div className="space-y-6">
      ...
      <PageHeader icon={<Layers className="w-7 h-7 text-indigo-600"/>} title={batch.name} subtitle={t("esign.view.batch_detail_subtitle", {
            signed: batch.signed,
            total: batch.total,
        })} actions={<div className="flex gap-2">
            {pending && batch.status !== "CANCELLED" && (<button onClick={run} disabled={running} className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white text-xs font-semibold px-4 py-2 rounded-lg flex items-center gap-2">
                <Play className="w-4 h-4"/>
                {running ? t("esign.message.batch_running") : t("esign.action.run_batch")}
              </button>)}
            {batch.status !== "CANCELLED" && batch.status !== "COMPLETED" && (<button onClick={cancelBatch} className="bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold px-4 py-2 rounded-lg flex items-center gap-2">
      ...
      {code && (<div className="bg-indigo-50 border border-indigo-200 rounded-xl p-5 text-center">
          ...
          <div className="flex justify-center gap-2 mt-3">
            {code.split("").map((digit, index) => (<span key={index} className="w-10 h-12 inline-flex items-center justify-center text-xl font-bold font-mono text-indigo-700 bg-white rounded-lg border border-indigo-200">
                {digit}
              </span>))}
          </div>
        </div>)}
      ...
      <Card title={t("esign.view.batch_documents")}>
        <table className="w-full text-left text-xs text-slate-600">
          ...
          <tbody className="divide-y divide-slate-100">
            {(batch.items ?? []).map((item, index) => (<tr key={item.id} className="hover:bg-slate-50">
      ...
      <div className="bg-white rounded-xl max-w-lg w-full p-6 shadow-xl border border-slate-200 max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={submit} className="flex-1 flex flex-col min-h-0 space-y-4">
          <div>
            ...
            <input id="batch-name" value={name} onChange={(event) => setName(event.target.value)} placeholder={t("esign.field.batch_name_placeholder")} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500" required/>
          </div>
          ...
          <div className="flex-1 min-h-0 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto border border-slate-200 rounded-lg divide-y divide-slate-100">
              {loading ? (<div className="p-4">
                  <Loading />
                </div>) : documents.length === 0 ? (<EmptyState message={t("esign.message.no_pending_documents")}/>) : (documents.map((doc) => (<label key={doc.id} className="flex items-center gap-3 px-3 py-2.5 hover:bg-slate-50 cursor-pointer">
                    <input type="checkbox" checked={picked.has(doc.id)} onChange={() => toggle(doc.id)} className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"/>
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="submit" disabled={busy || picked.size === 0 || !name.trim()} className="w-1/2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-medium py-2 rounded-lg text-xs">

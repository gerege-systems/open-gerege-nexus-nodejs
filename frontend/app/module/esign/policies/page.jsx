...
export default function EsignPoliciesPage() {
    ...
    const update = (patch) => setPolicy((current) => (current ? { ...current, ...patch } : current));
    ...
    if (loading)
        return <Loading />;
    if (!policy)
        return <Banner tone="error" message={error ?? t("base.message.error")}/>;
    return (<div className="space-y-6">
      <PageHeader icon={<ShieldCheck className="w-7 h-7 text-indigo-600"/>} title={t("esign.view.policies_title")} subtitle={t("esign.view.policies_subtitle")}/>
        ...
        <Card title={t("esign.view.policy_rails")}>
          <div className="p-4 space-y-4">
            <div>
              ...
              <select id="policy-provider" value={policy.default_provider} onChange={(event) => update({ default_provider: event.target.value })} disabled={policy.require_eid} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg bg-white focus:ring-2 focus:ring-indigo-500 disabled:bg-slate-50">
            ...
            <Toggle id="policy-require-eid" label={t("esign.field.require_eid")} hint={t("esign.field.require_eid_hint")} checked={policy.require_eid}
    ...
    onChange={(require_eid) =>
        ...
        update(require_eid ? { require_eid, default_provider: "EID" } : { require_eid })}/>
            ...
            <div>
              ...
              <select id="policy-level" value={policy.min_certificate_level} onChange={(event) => update({ min_certificate_level: event.target.value })} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg bg-white focus:ring-2 focus:ring-indigo-500">
        ...
        <Card title={t("esign.view.policy_rules")}>
          <div className="p-4 space-y-4">
            <Toggle id="policy-onbehalf" label={t("esign.field.allow_on_behalf_of")} hint={t("esign.field.allow_on_behalf_of_hint")} checked={policy.allow_on_behalf_of} onChange={(allow_on_behalf_of) => update({ allow_on_behalf_of })}/>
            <Toggle id="policy-selfsign" label={t("esign.field.allow_self_sign")} hint={t("esign.field.allow_self_sign_hint")} checked={policy.allow_self_sign} onChange={(allow_self_sign) => update({ allow_self_sign })}/>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <input id="policy-retention" type="number" min={0} value={policy.retention_days} onChange={(event) => update({ retention_days: Number(event.target.value) })} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500"/>
              ...
              <div>
                ...
                <input id="policy-upload" type="number" min={1} max={25} value={policy.max_upload_mb} onChange={(event) => update({ max_upload_mb: Number(event.target.value) })} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500"/>
        ...
        <button type="submit" disabled={saving} className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white text-xs font-semibold px-4 py-2 rounded-lg inline-flex items-center gap-1.5">
...
function Toggle({ id, label, hint, checked, disabled, onChange, }) {
    return (<label htmlFor={id} className={`flex items-start gap-3 ${disabled ? "opacity-50" : "cursor-pointer"}`}>
      <input id={id} type="checkbox" checked={checked} disabled={disabled} onChange={(event) => onChange(event.target.checked)} className="mt-0.5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"/>

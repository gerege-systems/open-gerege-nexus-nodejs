...
- **تأكيد البريد الإلكتروني** (`platform/emailverify`) — مسار موحَّد لإثبات ملكية
  عنوان: روابط تُستخدم مرة واحدة، ومفاتيح عملاء API لكل مستأجر، وحصص بالساعة،
  وقائمة سماح لعناوين العودة. تستدعيه وحدات التطبيقات داخل العملية، بينما
  تستدعيه الأنظمة الخارجية عبر `POST /api/v1/verify/send`. يُدار من الإعدادات ←
...
| `POST` | `/api/v1/verify/send` | إصدار رابط تأكيد البريد الإلكتروني (مفتاح عميل أو جلسة) |
| `GET` | `/api/v1/verify/confirm` | تنفيذ الرابط الوارد في الرسالة — يعمل مرة واحدة فقط |
| `GET/POST/PUT/DELETE` | `/api/v1/admin/email-verification/*` | نظرة عامة على التأكيدات وعملاء الـ API (مسؤول) |

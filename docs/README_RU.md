...
- **Подтверждение адреса электронной почты** (`platform/emailverify`) — единый
  процесс подтверждения: одноразовые ссылки, ключи API-клиентов для каждого
  арендатора, часовые лимиты и белый список адресов возврата. Модули приложений
  вызывают его внутри процесса, внешние системы — через
  `POST /api/v1/verify/send`. Управляется в разделе «Настройки → Подтверждение
  адреса».
...
| `POST` | `/api/v1/verify/send` | Выдать ссылку подтверждения адреса (ключ клиента или сессия) |
| `GET` | `/api/v1/verify/confirm` | Обработать ссылку из письма — срабатывает один раз |
| `GET/POST/PUT/DELETE` | `/api/v1/admin/email-verification/*` | Обзор подтверждений и API-клиенты (админ) |
